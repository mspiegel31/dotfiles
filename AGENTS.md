...
- Put OMP-specific subagents in `dot_agents/agents/`. Only `dot_omp/private_profiles/private_{personal,work}/private_agent/symlink_agents` exposes that directory at OMP's profile path `~/.omp/profiles/<name>/agent/agents/`.
...
- Put OMP extensions in `dot_agents/extensions/`. OMP does not auto-discover `.agents/extensions/`; `dot_omp/private_profiles/private_{personal,work}/private_agent/symlink_extensions` exposes it at the profile path `~/.omp/profiles/<name>/agent/extensions/`, the same way `symlink_agents` does for subagents.
...
- OMP runs under explicit named profiles: `omp-personal` / `omp-work` (zsh aliases for `omp --profile <name>`). Config lives in `dot_omp/private_profiles/private_{personal,work}/private_agent/`, rendered to `~/.omp/profiles/<name>/agent/`. Each profile is a complete, self-contained tree — profiles inherit nothing from each other or from the default root, so every setting (including common ones) must be present in the rendered profile config. Shared fragments live in `.chezmoitemplates/omp/` and are included at render time. Do not set ambient `OMP_PROFILE`, and do not leave config in the default `~/.omp/agent/` root: nothing in a profile reads it.
- Profile `config.yml` files (base + delta via `.chezmoitemplates/omp/config-base.yml.tmpl`) are the only source of `modelRoles`. Do not reintroduce a `PI_CONFIG_FILES` overlay: an overlay outranks the rendered config, so a role set there silently wins and the profile's value becomes dead text. Do not reintroduce hostname gating for OMP config — profile selection is the machine-identity boundary.
- `grafana-mcp-server` goes through `dot_omp/private_profiles/private_work/private_agent/bin/executable_grafana-mcp-proxy.mjs`, rendered to `~/.omp/profiles/work/agent/bin/`, not through `tsh mcp connect`. `tsh mcp connect` builds a per-client Teleport connection and reissues an app certificate on every start, so concurrent omp sessions and subagents blow the client's 30s initialize budget (`Connection to MCP server "grafana-mcp-server" timed out after 30000ms`, `Transport closed`) and omp then drops the server for the rest of the session. The wrapper keeps one shared detached `tsh proxy mcp` listener on 127.0.0.1:8888, validates the MCP endpoint before serving (a listener left from an expired Teleport session still accepts TCP), restarts a dead listener from a health check, and never tears the listener down on exit. An expired `tsh login` is still fatal — nothing local can reissue it.
