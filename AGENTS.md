...
- `dot_omp/agent/config.yml.tmpl` is the only source of `modelRoles`. Do not reintroduce a `PI_CONFIG_FILES` overlay: an overlay outranks the rendered config, so a role set there silently wins and the template's value becomes dead text. Host-gate the roles instead.
